...
int MPU9250::calibrateAccel2(){
  ...
  Serial.println("Accel calibration. Hold imu still on flat surface.");
  for (int c = 3; c > 0; c--){
  ...
  //check which axis is aligned with G vector
  int8_t whichax;   //indicates which axis along G vector
  if (abs(_axbD) > abs(_aybD) && abs(_axbD) > abs(_azbD)){
    _ayb = _aybD;
    _azb = _azbD;
    whichax = 0;
  }
  // yaxis
  if (abs(_aybD) > abs(_axbD) && abs(_aybD) > abs(_azbD)){
    _axb = _axbD;
    _azb = _azbD;
    whichax = 1;
  }
  // zaxis
  if (abs(_azbD) > abs(_aybD) && abs(_azbD) > abs(_axbD)){
    _ayb = _aybD;
    _axb = _axbD;
    whichax = 2;
  }
  Serial.println("whichax: " + String(whichax));
  Serial.println("Now turn IMU perpendicular in 5 seconds...");
  for (int c = 5; c > 0; c--){
    Serial.print(String(c) + "...");
    delay(1000);
  }
  Serial.println("\nCalibrating on new orientiation...");
